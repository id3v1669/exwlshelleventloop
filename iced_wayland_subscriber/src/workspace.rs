...
use crate::worker::{self, Bootstrap, Disposition, Worker};
    ...
    fn event(
        ...
        match event {
            ...
            ext_workspace_manager_v1::Event::Done => {
                state.published = state.snapshot.clone();
            ...
            ext_workspace_manager_v1::Event::Finished => {
                ...
                state.published.manager = None;
                state.published.groups.clear();
                state.published.workspaces.clear();
...
impl Workspaces {
    ...
    fn refresh_published_outputs(&mut self) {
        for group in &mut self.published.groups {
            group.outputs = group
                .bound_outputs
                .iter()
...
impl Worker for Workspaces {
    ...
    fn disposition(event: &WorkspaceEvent) -> Disposition {
        match event {
            ...
            // `finished` destroys the manager. The other two only ever precede
            // the worker returning. Nothing follows any of them.
            WorkspaceEvent::Finished | WorkspaceEvent::Unsupported | WorkspaceEvent::Stop(_) => {
    ...
    fn init(
        ...
        globals: &GlobalList,
        qh: &QueueHandle<Self>,
    ) -> Result<Bootstrap<Self>, Error> {
        let present = globals.contents().with_list(|globals| {
            globals
                .iter()
                .any(|global| global.interface == ExtWorkspaceManagerV1::interface().name)
        });
        if !present {
            return Ok(Bootstrap::Unavailable(WorkspaceEvent::Unsupported));
        }
