...
pub(crate) enum Bootstrap<S: Worker> {
    Ready(S),
    /// Deliver this one event and never run the dispatch loop.
    #[cfg_attr(not(feature = "workspace"), allow(dead_code))]
    Unavailable(S::Event),
}
...
enum Ended {
    ...
    Failed(Error),
    /// The compositor cannot serve this worker at all. Restarting cannot help.
    Unavailable,
}
...
fn run<S: Worker>(
    ...
    for attempt in 0..=MAX_RESTARTS {
        match attempt_once::<S>(&connection, &tx, &stop, &handoff) {
            Ended::Done | Ended::Unavailable => return,
            Ended::Failed(error) => {
...
fn attempt_once<S: Worker>(
    ...
    let started = (|| {
        ...
        let qh = queue.handle();
        let bootstrap = S::init(connection, &globals, &qh)?;
        Ok::<_, Error>((queue, qh, bootstrap))
    })();

    let (mut queue, qh, mut state) = match started {
        Ok((queue, qh, Bootstrap::Ready(state))) => (queue, qh, state),
        Ok((_, _, Bootstrap::Unavailable(event))) => {
