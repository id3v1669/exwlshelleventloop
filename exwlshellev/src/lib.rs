...
impl<T: 'static> WindowState<T> {
    ...
    fn can_remove_shell(&self, id: id::Id) -> bool {
        self.units
            .iter()
            .any(|unit| unit.id == id && unit.becreated)
    }
    ...
    fn event(
        ...
        match &event {
            wl_surface::Event::Enter { output } => {
                if !unit.wl_outputs.contains(output) {
                    unit.wl_outputs.push(output.clone());
                }
            }
            wl_surface::Event::Leave { output } if !matches!(unit.shell, Shell::LayerShell(..)) => {
                unit.wl_outputs.retain(|o| o != output);
            }
            _ => {}
        }
                        ...
                        match data {
                            ...
                            ReturnData::NewLayerShell((
                                ...
                                let output = match output_type {
                                    ...
                                    OutputOption::GlobalName(name) => {
                                        window_state.output_by_global_name(name)
                                    }
